...
Claude Code가 세션 시작 시 자동으로 wiki context를 주입하도록 훅을 설정한다.
    ...
    "SessionStart": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "python3 ~/your-path/my-brain/.hooks/session-start.py"
          }
        ]
      }
    ]
...
> 훅이 세션 시작 시 index.md(L0) + 진행 중인 프로젝트 3개(L1)를 자동 로드해준다.

### 6. 업데이트 명령어 등록 (선택사항, 강력 권장)

```bash
# ~/.zshrc에 추가
echo "alias mybrain-update='bash ~/your-path/my-brain/update.sh'" >> ~/.zshrc
source ~/.zshrc
```
...
CLAUDE.md와 훅 파일이 업데이트됐을 때:

```bash
mybrain-update
```

개인 wiki 데이터(wiki/, raw/, log.md 등)는 절대 건드리지 않고 구조 파일만 교체한다.
